...
12. Record a payment receipt through `metering_billing.PaymentSettlementService` from a stored projected payment intent.  Apply the exact received amount, reduce collection outstanding, and settle the case when remaining is zero.  Emit a cash journal proposal to AIS next, or record another partial receipt.  Do not capture via a provider, emit an `accounting_journal_proposal` from this path, or post journals.
