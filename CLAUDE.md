...
15. Let AIS pull persisted journal proposals through `GET /v1/journal-proposals` and `GET /v1/journal-proposals/{proposal_id}`.  Require a tenant on every read.  Do not mark exported, posted, or consumed.  AIS pulls validated proposals and owns `posting_receipt`.
