...
test("first-at accepted observation shows exact remaining amount, enqueue, and wait", () => {
  const observation = loadFixture("accepted_at_signal.json");
  const outbox = loadFixture("pending_spend_budget_approaching.json");
  const html = renderSpendBudgetApproaching(observation, [outbox]);
  assert.match(html, /0\.003705 USD/);
  assert.match(html, /0 USD/);
  assert.match(html, /oc-status-chip--settled/);
  assert.match(html, /at/);
  assert.match(html, /accepted/);
  assert.match(html, /spend_budget\.approaching/);
  assert.match(html, /019d7b92-1aa0-7a7f-b61c-962c0f4bfe02/);
  assert.match(html, />Wait</);
  assert.match(html, /Observe the spend budget approaching signal, then wait/);
  assert.equal(
    SPEND_BUDGET_APPROACHING_CUSTOMER_COPY,
    "Observe the spend budget approaching signal, then wait.",
  );
  assert.equal(SPEND_BUDGET_APPROACHING_TENANT_REFERENCE, "urn:cwl:tenant_001");
  assert.equal(observation.spend_budget_approaching_signal_outcome_code, "accepted");
  assert.equal(observation.utilization_status, "at");
  assert.equal(observation.remaining_amount, "0");
  assert.equal(observation.budget_amount, "0.003705");
  assert.equal(observation.next_operator_action, "wait");
  assert.equal(typeof observation.remaining_amount, "string");
  assert.equal(typeof observation.budget_amount, "string");
  assert.notEqual(typeof observation.remaining_amount, "number");
  assert.equal(outbox.event_type_code, "spend_budget.approaching");
  assert.equal(outbox.source_id, observation.spend_budget_id);
  assert.equal(outbox.delivery_status, "pending");
  assert.match(html, /urn:cwl:tenant_001/);
  assert.match(
  ...
  assert.match(renderAmountDue({
    amount_due: observation.remaining_amount,
    currency_code: observation.currency_code,
  }), /0 USD/);
  assert.match(renderAmountDue({
    amount_due: observation.budget_amount,
  ...
  assert.ok(!("over_amount" in observation));
  assert.ok(!("rated_amount" in observation));
  assert.ok(!("card_pan" in observation));
  assert.ok(!("retained_earnings" in observation));
  assert.ok(!("statutory_account_id" in observation));
...
test("under approaching observation waits with zero approaching-signal rows", () => {
  const observation = loadFixture("accepted_under_approaching_signal.json");
  const html = renderSpendBudgetApproaching(observation, []);
  assert.match(html, /100\.00 USD/);
  assert.match(html, /99\.996295 USD/);
  assert.match(html, /oc-status-chip--settled/);
  assert.match(html, /under/);
  assert.match(html, /accepted/);
  assert.match(html, />Wait</);
  assert.match(html, /Approaching-signal rows<\/span><p class="oc-invoice-statement__id">0<\/p>/);
  assert.doesNotMatch(html, /spend_budget\.approaching/);
  assert.equal(observation.utilization_status, "under");
  assert.equal(observation.remaining_amount, "99.996295");
  assert.equal(observation.budget_amount, "100.00");
  assert.equal(observation.next_operator_action, "wait");
  assert.equal(typeof observation.remaining_amount, "string");
  assert.notEqual(typeof observation.remaining_amount, "number");
  ...
  const missingRows = renderSpendBudgetApproaching(observation);
  assert.match(missingRows, /Approaching-signal rows<\/span><p class="oc-invoice-statement__id">0<\/p>/);
  assert.doesNotMatch(missingRows, /spend_budget\.approaching/);
  assert.ok(!("over_amount" in observation));
  assert.ok(!("card_pan" in observation));
});

test("duplicate replay keeps first-at-wins outbox and current at evaluation", () => {
  const observation = loadFixture("duplicate_replay_approaching_signal.json");
  const outbox = loadFixture("pending_spend_budget_approaching.json");
  const firstAt = loadFixture("accepted_at_signal.json");
  const html = renderSpendBudgetApproaching(observation, [outbox]);
  assert.match(html, /0\.003705 USD/);
  assert.match(html, /duplicate_replay/);
  assert.match(html, /spend_budget\.approaching/);
  assert.match(html, /Approaching-signal rows<\/span><p class="oc-invoice-statement__id">1<\/p>/);
  assert.match(html, />Wait</);
  assert.equal(observation.spend_budget_id, firstAt.spend_budget_id);
  assert.equal(observation.remaining_amount, firstAt.remaining_amount);
  assert.equal(observation.budget_amount, firstAt.budget_amount);
  assert.equal(observation.utilization_status, "at");
  assert.equal(observation.spend_budget_approaching_signal_outcome_code, "duplicate_replay");
  assert.equal(outbox.source_id, firstAt.spend_budget_id);
  assert.equal(typeof observation.remaining_amount, "string");
  assert.match(html, /urn:cwl:tenant_001/);
  assert.ok(!("over_amount" in observation));
  assert.ok(!("card_pan" in observation));
...
test("unused issued credit note void shows exact voided amount and wait", () => {
  ...
  const applied = loadFixture("applied_morning_credit_note.json");
  const html = renderIssuedCreditNoteVoid(statement);
  assert.match(html, /11\.00 USD/);
  assert.match(html, /recorded/);
  assert.match(html, />Wait</);
  assert.match(html, /Void an unused issued credit note, then wait/);
  assert.equal(
    ISSUED_CREDIT_NOTE_VOID_CUSTOMER_COPY,
    "Void an unused issued credit note, then wait.",
  );
  assert.equal(nextIssuedCreditNoteVoidActionCopy("wait"), "Wait");
  assert.equal(nextIssuedCreditNoteVoidActionCopy("void"), "Void unused credit note");
  assert.equal(statement.next_operator_action, "wait");
  assert.equal(statement.issued_credit_note_void_status, "recorded");
  assert.equal(statement.voided_amount, "11.00");
  assert.equal(statement.issued_credit_note_id, taxedIssued.issued_credit_note_id);
  assert.notEqual(statement.issued_credit_note_id, applied.issued_credit_note_id);
  assert.equal(typeof statement.voided_amount, "string");
  assert.notEqual(typeof statement.voided_amount, "number");
  assert.match(renderTenantPin(statement), /urn:cwl:tenant_001/);
  assert.match(renderAmountDue({
  ...
  assert.ok(!("issued_credit_note_void_outcome_code" in statement));
  assert.ok(!("issued_credit_note_lines" in statement));
  assert.ok(!("card_pan" in statement));
  assert.ok(!("legal_invoice_number" in statement));
  assert.ok(!("retained_earnings" in statement));
...
test("validated morning leftover journal shows exact leftover debit and wait", () => {
  const statement = loadFixture("validated_morning_leftover_journal.json");
  const parkedLeftover = loadFixture("parked_morning_unapplied_cash.json");
  const cashJournal = loadFixture("validated_morning_cash_journal.json");
  const html = renderJournalProposal(statement);
  assert.match(html, /0\.001 USD/);
  assert.match(html, /validated/);
  ...
  assert.equal(statement.lines[0].account_role_code, "cash_receipt");
  assert.equal(statement.lines[0].debit_amount, parkedLeftover.unapplied_amount);
  assert.equal(statement.lines[0].credit_amount, "0");
  assert.equal(statement.lines[1].account_role_code, "unapplied_cash");
  assert.equal(statement.lines[1].debit_amount, "0");
  assert.equal(statement.lines[1].credit_amount, parkedLeftover.unapplied_amount);
  assert.equal(statement.lines.length, 2);
  assert.equal(
    statement.source_event_references[0],
    `urn:cwl:tenant_001:unapplied_cash:${parkedLeftover.unapplied_cash_id}`,
  );
  assert.equal(parkedLeftover.unapplied_cash_status, "parked");
  assert.notEqual(statement.proposal_id, cashJournal.proposal_id);
  assert.ok(!("next_operator_action" in statement));
  assert.equal(typeof statement.lines[0].debit_amount, "string");
  assert.notEqual(typeof statement.lines[0].debit_amount, "number");
...
test("validated morning leftover-apply journal shows exact unapplied-cash debit and wait", () => {
  const statement = loadFixture("validated_morning_leftover_apply_journal.json");
  const leftoverApply = loadFixture("applied_morning_unapplied_cash.json");
  const leftoverJournal = loadFixture("validated_morning_leftover_journal.json");
  const html = renderJournalProposal(statement);
  assert.match(html, /0\.001 USD/);
  assert.match(html, /validated/);
  assert.match(html, />Wait</);
  ...
  assert.equal(statement.proposal_status, "validated");
  assert.equal(journalProposalAmount(statement), "0.001");
  assert.equal(statement.lines[0].account_role_code, "unapplied_cash");
  assert.equal(statement.lines[0].debit_amount, leftoverApply.applied_amount);
  assert.equal(statement.lines[0].credit_amount, "0");
  assert.equal(statement.lines[1].account_role_code, "accounts_receivable");
  assert.equal(statement.lines[1].debit_amount, "0");
  assert.equal(statement.lines[1].credit_amount, leftoverApply.applied_amount);
  assert.equal(statement.lines.length, 2);
  assert.equal(
    statement.source_event_references[0],
    `urn:cwl:tenant_001:unapplied_cash_application:${leftoverApply.unapplied_cash_application_id}`,
  );
  assert.equal(leftoverApply.remaining_outstanding_amount, "19.999");
  assert.equal(leftoverApply.unapplied_cash_application_status, "applied");
  assert.notEqual(statement.proposal_id, leftoverJournal.proposal_id);
  assert.ok(!("next_operator_action" in statement));
  assert.equal(typeof statement.lines[0].debit_amount, "string");
  assert.notEqual(typeof statement.lines[0].debit_amount, "number");
  assert.match(renderTenantPin(statement), /urn:cwl:tenant_001/);
...
test("validated morning leftover-refund journal shows exact unapplied-cash debit and wait", () => {
  const statement = loadFixture("validated_morning_leftover_refund_journal.json");
  const leftoverRefund = loadFixture("refunded_morning_unapplied_cash.json");
  const leftoverApply = loadFixture("applied_morning_unapplied_cash.json");
  const leftoverApplyJournal = loadFixture("validated_morning_leftover_apply_journal.json");
  const html = renderJournalProposal(statement);
  assert.match(html, /0\.001 USD/);
  assert.match(html, /validated/);
  assert.match(html, />Wait</);
  ...
  assert.equal(journalProposalAmount(statement), "0.001");
  assert.equal(statement.lines[0].account_role_code, "unapplied_cash");
  assert.equal(statement.lines[0].debit_amount, leftoverRefund.refund_amount);
  assert.equal(statement.lines[0].credit_amount, "0");
  assert.equal(statement.lines[1].account_role_code, "cash_receipt");
  assert.equal(statement.lines[1].debit_amount, "0");
  assert.equal(statement.lines[1].credit_amount, leftoverRefund.refund_amount);
  assert.equal(statement.lines.length, 2);
  assert.equal(
    statement.source_event_references[0],
    `urn:cwl:tenant_001:unapplied_cash_refund:${leftoverRefund.unapplied_cash_refund_id}`,
  );
  assert.equal(leftoverRefund.unapplied_cash_status, "parked");
  assert.equal(leftoverApply.remaining_outstanding_amount, "19.999");
  assert.notEqual(statement.proposal_id, leftoverApplyJournal.proposal_id);
  assert.ok(!("next_operator_action" in statement));
  assert.equal(typeof statement.lines[0].debit_amount, "string");
  assert.notEqual(typeof statement.lines[0].debit_amount, "number");
  assert.match(renderTenantPin(statement), /urn:cwl:tenant_001/);
...
test("validated unused invoice-void journal shows exact usage-revenue debit and wait", () => {
  const statement = loadFixture("validated_unused_invoice_void_journal.json");
  const unusedInvoiceVoid = loadFixture("voided_unused_issued_invoice.json");
  const taxedIssued = loadFixture("issued_taxed_hundred.json");
  const assessedVat = loadFixture("assessed_morning_vat.json");
  ...
  assert.match(html, /Let AIS pull the validated journal/);
  assert.equal(statement.proposal_status, "validated");
  assert.equal(journalProposalAmount(statement), "100.00");
  assert.equal(statement.lines[0].account_role_code, "usage_revenue");
  assert.equal(statement.lines[0].debit_amount, taxedIssued.tax_exclusive_amount);
  assert.equal(statement.lines[0].credit_amount, "0");
  assert.equal(statement.lines[1].account_role_code, "tax_payable");
  assert.equal(statement.lines[1].debit_amount, taxedIssued.tax_amount);
  assert.equal(statement.lines[1].credit_amount, "0");
  assert.equal(statement.lines[2].account_role_code, "accounts_receivable");
  assert.equal(statement.lines[2].debit_amount, "0");
  assert.equal(statement.lines[2].credit_amount, unusedInvoiceVoid.voided_amount);
  assert.equal(statement.lines.length, 3);
  assert.equal(
    statement.source_event_references[0],
