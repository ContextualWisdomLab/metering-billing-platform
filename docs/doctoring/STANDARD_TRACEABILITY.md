...
| RFC 9110 | At-least-once delivery is made safe by idempotent ingest of the same source key and payload. The HTTP accept surface maps `accepted` and `duplicate_replay` to 200, `rejected` to 422, and unknown routes to 404. GET journal-proposal query is a safe, idempotent pull and does not mutate `proposal_status`. POST `/v1/posting-receipt-observations` is the operator trigger that pulls an AIS receipt; GET of a stored observation is a safe read and does not call AIS. POST `/v1/credit-adjustments` records a commercial credit; GET of a stored credit is a safe tenant-scoped read and does not call AIS. POST `/v1/rate-cards` publishes an append-only catalog version; GET list, card, versions, and version routes are safe tenant-scoped reads and return 404 across tenants. |
...
| IFRS 15 | Billing and revenue recognition remain separate; an invoice-intent draft, journal proposal, collection reminder, and commercial credit adjustment are consideration evidence, not proof that revenue has been earned or reversed. A later price concession or billing correction is variable consideration. Performance-obligation and principal-versus-agent judgments belong to versioned accounting policy in AIS. |
