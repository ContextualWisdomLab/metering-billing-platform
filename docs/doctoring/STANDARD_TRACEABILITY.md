...
| RFC 9110 | At-least-once delivery is made safe by idempotent ingest of the same source key and payload. The HTTP accept surface maps `accepted` and `duplicate_replay` to 200, `rejected` to 422, and unknown routes to 404. GET journal-proposal query is a safe, idempotent pull and does not mutate `proposal_status`. POST `/v1/posting-receipt-observations` is the operator trigger that pulls an AIS receipt; GET of a stored observation is a safe read and does not call AIS. |
