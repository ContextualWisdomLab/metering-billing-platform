...
`metering_billing.PaymentReceiptPresentmentService` projects a stored payment receipt into a commercial statement. PCI DSS keeps card PAN off this path (PCI Security Standards Council, 2024). `POST /v1/payment-receipts`, `GET /v1/payment-receipts/{payment_receipt_id}`, and `GET /v1/payment-receipts` are tenant-scoped (Fielding et al., 2022; Google, 2024). Accept enqueues `payment_receipt.applied` and does not flip `proposal_status`. `operator_console` Storybook presents the received amount and the next action. Record the receipt, then drain or wait for AIS to pull the cash journal.
