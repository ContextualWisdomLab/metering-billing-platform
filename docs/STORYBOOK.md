...
Customer copy on every journal proposal: validated cash or invoice-draft amount and the next operator action. Let AIS pull the validated journal.
...
| JournalProposal | `src/journal_proposal.js` | `validated_morning_cash_journal.json`, `validated_morning_invoice_draft_journal.json`, `validated_taxed_invoice_draft_journal.json` |
