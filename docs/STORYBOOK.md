...
Customer copy on every journal proposal: validated cash amount and the next operator action. Let AIS pull the validated journal.
...
| JournalProposal | `src/journal_proposal.js` | `validated_morning_cash_journal.json` |
