...
Assignments and capabilities use `valid_from`, `valid_to`, and `recorded_at`. Closing an interval supersedes a fact; it does not erase history. Composite foreign keys bind credentials, principals, billing accounts, and usage to the same tenant. PostgreSQL migration `0036` enforces the credential half-open non-overlap rule with an exclusion constraint and keeps proposal references unique per tenant.
...
A stored spend budget is identified by `(tenant_account_id, billing_account_id, window_started_at, window_ended_at, currency_code, source_payload_hash, spend_budget_contract_version)`.  Internal primary key is the opaque generated `spend_budget_id`.  The hash covers the billing account, ISO 4217 currency, exact `budget_amount` greater than zero, UTC window instants, and contract version.  Status is `published` only.  A later distinct amount or hash for the same account, window, and currency inserts a new row.  A published budget is never updated.  The row invents no rated-spend comparison, journal, webhook, reservation, or statutory number.  `GET /v1/spend-budgets/{spend_budget_id}` projects the stored row.  `GET /v1/spend-budgets` lists `{spend_budgets, next_cursor}` ordered by `published_at` then `spend_budget_id`.
