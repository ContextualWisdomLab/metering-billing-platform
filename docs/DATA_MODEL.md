...
A stored collection dispute is identified by `(tenant_account_id, collection_case_id)`.  Internal primary key is the opaque generated `collection_dispute_id`.  The hash covers the case, invoice draft, optional issued invoice, currency, remaining snapshot, and contract version.  Status is `held` only.  Remaining outstanding is a snapshot of current remaining and is not changed.  One case holds at most once through this command.  Case status becomes `disputed`.  The hold row invents no payment receipt, credit note, settlement, write-off, void, journal, or webhook.  Release is a later slice.
