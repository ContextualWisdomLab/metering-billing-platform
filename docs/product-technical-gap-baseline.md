...
| Foundation candidate | PR #1, `agent/initial-billing-foundation`, last verified code head `cded38a`; its exact-head repository-contract check is the first gate. |
| Latest feature candidate | PR #82, `cursor/spend-budget-publish-f556`, last verified head `5c64f9d`; it contains the cumulative Python package, PostgreSQL migrations, HTTP surface, operator-console Storybook, schemas, tests, dependency repairs, outbound URL validation, hollow-resolver hardening, boolean JSON Schema validator support, reusable-workflow reference validation, persistence attribution constraints, uv project metadata, and matching data-model documentation. |
