...
| Latest feature candidate | PR #82, `cursor/spend-budget-publish-f556`, last verified head `1fc169d`; it contains the cumulative Python package, PostgreSQL migrations, HTTP surface, operator-console Storybook, schemas, tests, dependency repairs, outbound URL validation, hollow-resolver hardening, boolean JSON Schema validator support, reusable-workflow reference validation, persistence attribution constraints, and matching data-model documentation. |
