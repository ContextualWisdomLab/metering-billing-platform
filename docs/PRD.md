...
- When a journal proposal is validated, a payment receipt is applied, a credit is recorded, an invoice is issued, a credit note is issued, a credit note is applied, a collection case is settled, leftover remaining is written off, or parked leftover is applied, one `webhook_outbox_event` is appended. Replay of the commercial fact does not enqueue a second row.
- Event types in this slice are `journal_proposal.validated`, `payment_receipt.applied`, `credit_adjustment.recorded`, `invoice.issued`, `credit_note.issued`, `credit_note.applied`, `collection.settled`, `write_off.recorded`, and `unapplied_cash.applied`. Journal, payment, and credit payloads wrap the published contract. `invoice.issued`, `credit_note.issued`, `credit_note.applied`, `collection.settled`, `write_off.recorded`, and `unapplied_cash.applied` `data` are thin references plus hash and omit lines, PAN, secrets, and statutory identifiers.
