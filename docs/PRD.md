...
- `late_usage`, `correction`, and `reversal` are the only adjustment kinds.
  ...
  uppercase ISO 4217 code, and the tenant-scoped source reference is the stable
  replay identity with the source payload hash included in the payload identity.
- An identical replay, including one with a regenerated opaque ID, returns the
...
- When a journal proposal is validated, a payment receipt is applied, a credit is recorded, an invoice is issued, a credit note is issued, a credit note is applied, a collection case is settled, leftover remaining is written off, parked leftover is applied, unused parked leftover is refunded, a collection case is held as disputed, a held dispute is released, a spend budget is published, a published spend budget is first observed as over, or a published spend budget is first observed as at the documented `budget_amount`, one `webhook_outbox_event` is appended. Replay of the commercial fact does not enqueue a second row.
- Event types in this slice are `journal_proposal.validated`, `payment_receipt.applied`, `credit_adjustment.recorded`, `invoice.issued`, `invoice.voided`, `credit_note.issued`, `credit_note.voided`, `credit_note.applied`, `collection.settled`, `write_off.recorded`, `unapplied_cash.applied`, `refund.recorded`, `dispute.held`, `dispute.released`, `spend_budget.published`, `spend_budget.over`, and `spend_budget.approaching`. Journal, payment, and credit payloads wrap the published contract. `invoice.issued`, `invoice.voided`, `credit_note.issued`, `credit_note.voided`, `credit_note.applied`, `collection.settled`, `write_off.recorded`, `unapplied_cash.applied`, `refund.recorded`, `dispute.held`, `dispute.released`, `spend_budget.published`, `spend_budget.over`, and `spend_budget.approaching` `data` are thin references plus hash and omit lines, PAN, secrets, and statutory identifiers.
...
- A known tenant and `billing_account_id` present one statement grouped by `currency_code` with exact inclusive `issued_invoice_total`, `voided_invoice_total`, `open_collection_remaining`, `applied_credit_total`, `voided_credit_total`, `write_off_total`, `parked_unapplied_cash`, and `refunded_unapplied_cash`. Currencies are never mixed in one sum. `issued_invoice_total` stays the issued snapshot. `applied_credit_total` stays applied credits only. Unused issued-invoice voids and unused issued-credit-note voids are separate buckets.
...
- A known tenant, `billing_account_id`, and half-open ISO 8601 window present already-rated spend grouped by `(currency_code, product_code)`. Optional `group_by=project` presents one row per `(currency_code, product_code, project_reference)` whose `project_reference` is the stored exclusive-account usage URN. Usage without `project_reference` is omitted from the project grouping. Mixed projects omit the run. Optional `group_by=credential` presents one row per `(currency_code, product_code, credential_reference)` whose `credential_reference` is the stored exclusive-account usage-event credential URN. Usage without `credential_reference` is omitted from the credential grouping. Mixed credentials omit the run. Optional `group_by=principal` presents one row per `(currency_code, product_code, billing_principal_reference)` whose `billing_principal_reference` is the stored exclusive-account usage-event principal URN. Mixed or unresolved principals omit the run. Optional `group_by=cost_center` presents one row per `(currency_code, product_code, cost_center_reference)` whose `cost_center_reference` is the stored exclusive-account usage-event cost-center URN. Usage without `cost_center_reference` is omitted from the cost-center grouping. Mixed cost centers omit the run. Each row carries exact `rated_amount` from stored rating-run or exclusive invoice-draft lines. Currencies are never mixed in one sum. The read does not re-rate, invent a unit price, or include unrated usage.
...
- A known tenant can publish one append-only commercial `spend_budget` for one same-tenant billing account, half-open ISO 8601 window, and ISO 4217 currency. `budget_amount` is an exact decimal greater than zero. Identity is `(tenant_account_id, billing_account_id, window_started_at, window_ended_at, currency_code, source_payload_hash, spend_budget_contract_version)`. Internal primary key is opaque generated `spend_budget_id`. Replay of the same identity returns the same `spend_budget_id` as `duplicate_replay` and does not grow the store. A later distinct amount or hash appends a new row. Status is `published` only. `spend_budget_id` is not a statutory number.
- `POST /v1/billing-accounts/{billing_account_id}/spend-budgets` is HTTP 200 for accepted and replayed writes. Missing tenant is HTTP 422. Unknown billing account is HTTP 404. Cross-tenant account is HTTP 403. Float money, zero or negative amount, illegal window, illegal currency, illegal cursor, and illegal `page_limit` fail closed. PAN, CVC, and provider secrets are refused.
- A known stored `spend_budget` presents identity, billing account, window, currency, exact `budget_amount`, `published_at`, `spend_budget_status`, and `next_operator_action` (`wait`).
- `GET /v1/spend-budgets/{spend_budget_id}` is HTTP 200 for the same tenant. Cross-tenant or unknown is HTTP 404 with no leak.
...
- First successful publish enqueues one existing #24 `spend_budget.published` outbox event. `source_id` is `spend_budget_id`. Replay does not enqueue a second row. Rejected publish writes zero outbox rows. HMAC, SSRF, and delivery contracts stay unchanged.
...
- `POST /v1/spend-budgets/{spend_budget_id}/over-signal` is HTTP 200 for accepted and replayed writes on same-tenant published budgets. Unknown or cross-tenant is HTTP 404 with no leak. Missing tenant is HTTP 422. A stored budget whose billing account belongs to another commercial `tenant_account` is HTTP 403. PAN, CVC, and provider secrets are refused.
- Envelope `data` is a thin reference plus hash: ids, contract version, `source_payload_hash`, currency, exact `budget_amount`, exact `over_amount`, window instants, `spend_budget_status`, and `utilization_status=over`. Remaining, rated lines, PII, PAN, secrets, raw documents, and statutory identifiers are omitted so currencies stay unmixed.
- Existing subscriptions opt in by including `spend_budget.over`. HMAC, SSRF, and delivery contracts stay unchanged.
- Operators observe first-over utilization, then run deliveries. GET evaluation, GET budget-status, persist, Storybook, and `spend_budget.published` stay unchanged. The write does not persist an evaluation snapshot, mutate the budget, hard-stop rating, ingest, or invoice draft, emit a journal, call AIS, or invent a dimension-scoped budget.
- A known stored `spend_budget` presents the live over-signal observation plus zero or one stored `spend_budget.over` webhook-outbox presentment. The read reuses `SpendBudgetEvaluationPresentmentService` and the existing over-signal envelope. `GET /v1/spend-budgets/{spend_budget_id}/over-signal` is HTTP 200 for the same tenant. Cross-tenant or unknown is HTTP 404 with no leak. Missing tenant is HTTP 422. A stored budget whose billing account belongs to another commercial `tenant_account` is HTTP 403. Float money fails closed. Replay is a safe GET and writes no outbox row.
- Operators inspect the live over-signal and stored first-over outbox row, then wait. The read does not persist, mutate the budget, hard-stop rating, ingest, or invoice draft, emit a webhook or journal, call AIS, invent a statutory identifier, or add a VAT/NTS adapter. Atomic authorization and quotas remain a later slice.

## Spend-budget-approaching-signal acceptance

- A known tenant can observe one same-tenant published `spend_budget` for approaching utilization. The write reuses `SpendBudgetEvaluationPresentmentService` so remaining/over math matches one-budget evaluation. The documented notify threshold is exact `budget_amount` via `utilization_status=at`. First observation with `utilization_status=at` enqueues one existing #24 `spend_budget.approaching` outbox event. `source_id` is `spend_budget_id`. Replay of the same at observation is `duplicate_replay` and does not enqueue a second row. under and over write zero approaching-signal outbox rows. Rejected observe writes zero rows.
- `POST /v1/spend-budgets/{spend_budget_id}/approaching-signal` is HTTP 200 for accepted and replayed writes on same-tenant published budgets. Unknown or cross-tenant is HTTP 404 with no leak. Missing tenant is HTTP 422. A stored budget whose billing account belongs to another commercial `tenant_account` is HTTP 403. PAN, CVC, and provider secrets are refused.
- Envelope `data` is a thin reference plus hash: ids, contract version, `source_payload_hash`, currency, exact `budget_amount`, exact `remaining_amount`, window instants, `spend_budget_status`, and `utilization_status=at`. Over, rated lines, PII, PAN, secrets, raw documents, and statutory identifiers are omitted so currencies stay unmixed.
- Existing subscriptions opt in by including `spend_budget.approaching`. HMAC, SSRF, and delivery contracts stay unchanged.
- Operators observe first-at utilization, then wait. The write does not persist an evaluation snapshot, mutate the budget, hard-stop rating, ingest, or invoice draft, emit a journal, call AIS, invent a statutory identifier, add a VAT/NTS adapter, or invent a default notify percentage. Atomic authorization and quotas remain a later slice.
- A known stored `spend_budget` presents the live approaching-signal observation plus zero or one stored `spend_budget.approaching` webhook-outbox presentment. The read reuses `SpendBudgetEvaluationPresentmentService` and the existing approaching-signal envelope. `GET /v1/spend-budgets/{spend_budget_id}/approaching-signal` is HTTP 200 for the same tenant. Cross-tenant or unknown is HTTP 404 with no leak. Missing tenant is HTTP 422. A stored budget whose billing account belongs to another commercial `tenant_account` is HTTP 403. Float money fails closed. Replay is a safe GET and writes no outbox row.
- Operators inspect the live approaching-signal and stored first-at outbox row, then wait. The read does not persist, mutate the budget, hard-stop rating, ingest, or invoice draft, emit a webhook or journal, call AIS, invent a statutory identifier, or add a VAT/NTS adapter. Atomic authorization and quotas remain a later slice.
...
- Operators inspect remaining, then wait. The read does not persist, mutate the budget, hard-stop rating, ingest, or invoice draft, emit a webhook or journal, call AIS, invent a statutory identifier, or add a VAT/NTS adapter. Atomic authorization and quotas remain a later slice. POST spend-budgets, one-budget evaluation GET, and #24 outbox stay unchanged.
- `operator_console` Storybook renders those exact-decimal under, at, and over rows with tokenized amount due, status chip, and tenant pin. Fixtures are one same-tenant account covering under and over (and at) plus a keyset `next_cursor` page. There is no login wall, Stripe, AIS call, or production SPA.
