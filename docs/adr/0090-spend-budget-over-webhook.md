...
- under and at write zero over-signal outbox rows. Replay of the same `spend_budget_id` source is `duplicate_replay` and does not enqueue a second row. The replay result is the current evaluation; the outbox row keeps the first-over facts. A crash after compute and before enqueue is healed by the next observe. Rejected observe writes zero rows.
