...
- When a commercial fact is accepted (not replayed), append one `webhook_outbox_event` for `journal_proposal.validated`, `payment_receipt.applied`, or `credit_adjustment.recorded`.  The payload is a thin envelope `{event_type_code, occurred_at, tenant_reference, data}` wrapping the existing published contract.  API secrets and PANs are refused.
