...
HTTP must expose the existing `tenant_api_credential` store.  It must not reconstruct a secret or reopen the one-time #22 bootstrap window, or invent rotation.
...
- Keep the #22 authorize rule: after any credential history exists, every `/v1` call, including issue, requires that key.  Only the first issue for a tenant with no history keeps the tenant pin.

## Consequences

- Operators issue a key, then send it on every `/v1` call; revoke when leaked.
- AIS `X-CWL-Tenant-Reference` pull stays bootstrap only until the first key is issued; revocation does not restore it.
