# ADR 0120: Operator Leftover-Apply Journal Storybook

**Status:** Accepted

## Context

#61 / ADR 0058 composes one validated leftover-apply
`accounting_journal_proposal` from a stored leftover apply. AIS pulls that
row through existing `GET /v1/journal-proposals` and
`GET /v1/journal-proposals/{proposal_id}`. #124 / ADR 0113 persists that
proposal so GET presentment survives process restart. `operator_console`
already presents validated cash, invoice-draft, and leftover journals
through the existing `JournalProposal` module, plus tokenized `AmountDue`,
`StatusChip`, and tenant-pin modules, but `STORYBOOK.md` has no
leftover-apply journal fixture. #116 already presents the leftover-apply
row. Existing leftover-apply UnappliedCash Storybook stays that leftover-apply
row presentment.
...
authorization stay later. Leftover-apply journal HTTP compose, GET,
...
- Ship one Storybook story for one same-tenant validated leftover-apply
  ...
  and leftover `JournalProposal` stories as those presentments. Keep
  existing leftover-apply UnappliedCash Storybook as the leftover-apply
  row presentment.
- Fixture lines debit `unapplied_cash` and credit `accounts_receivable`
  for the applied leftover exact string `0.001`. `proposal_status` stays
  `validated`. Leftover-apply remaining stays `19.999`. Next operator
  action copy is `wait`. The published journal-proposal contract has no
  `next_operator_action` field. Float money fails closed.
- Pin fixture `X-CWL-Tenant-Reference` to the commercial `tenant_account`
  (`urn:cwl:tenant_001`). Do not auto-create tenants. The leftover-apply
  journal `source_event_references` leftover-apply matches the existing
  leftover-apply UnappliedCash fixture.
- Do not invent BudgetGauge, a notify percentage, or a new money widget.
  `AmountDue` already presents the exact leftover-apply debit string.
- Do not change
  ...
  persist.
- Do not implement leftover-refund journal Storybook, write-off journal
  Storybook, unused void journal Storybook, atomic authorization, quotas,
  ...
  310100, or statutory identifier. Do not re-apply, refund, settle, or
  void. Do not flip `proposal_status` to `posted`.

## Consequences

- Operators can open one validated leftover-apply journal in Storybook and
  see exact unapplied-cash debit, AR credit, validated status, tenant pin,
  and the next action: wait. Cash, invoice-draft, and leftover journal
  stories stay those presentments. Leftover-apply UnappliedCash Storybook
  stays the leftover-apply row presentment. Leftover-apply remaining stays
  `19.999`.
- Python remains the commercial authority. The console only presents
  stored #61 JSON.
- Leftover-apply journal persist, HTTP write, GET, and #24 outbox stay
