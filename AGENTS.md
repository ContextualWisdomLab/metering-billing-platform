...
- Never store card data, PAT plaintext, prompt text, response text, provider secrets, or tenant API credential plaintext.  Persist only a keyed HMAC on `tenant_api_credential`.
