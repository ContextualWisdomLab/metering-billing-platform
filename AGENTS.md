...
- Let AIS pull persisted proposals through `GET /v1/journal-proposals` and `GET /v1/journal-proposals/{proposal_id}`.  Require a tenant on every read.  Cash and AR proposals share `journal_proposal` and appear in the same list.  Query never mutates `proposal_status` and never emits `posted`.
