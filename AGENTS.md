...
- Record a commercial credit through `metering_billing.CreditAdjustmentService.record_credit_adjustment` so a tenant, `invoice_draft_id`, exact `credit_amount`, `credit_reason_code`, source-payload hash, and contract version replay the same `credit_adjustment_id` and `proposal_id`.  Credits stay `recorded`.  If a collection case exists, reduce outstanding by the same amount and settle at zero.  Emit one validated revenue-debit / AR-credit journal proposal.  Do not post, call AIS, or invent a new account role.  Record the credit, then let AIS pull the validated proposal.
...
- Publish a tenant-scoped tax rate through `metering_billing.TaxRateService.publish_tax_rate` so a tenant, closed `tax_code`, exact `tax_rate` in `[0, 1]`, and contract version replay the same `tax_rate_version`.  Assess a stored draft through `metering_billing.TaxAssessmentService.assess_tax`.  Round tax half-even to the documented ISO 4217 minor units.  Reject assess after a collection case is open.  Collection outstanding uses `tax_inclusive_amount` when an assessment exists.  A taxed journal proposal credits semantic `tax_payable`; AIS must map that role.  Credit remaining is inclusive; tax-payable unwind is next.  Publish a tax rate, assess the draft, then propose the journal and let AIS pull.
