...
from test_spend_budget import BUDGET_AMOUNT, seed_account_ledger
...
class WebhookOutboxTests(unittest.TestCase):
    ...
    def test_spend_budget_published_enqueues_once_and_delivers_signed(self) -> None:
        """First publish enqueues spend_budget.published once; replay heals."""
        ledger, billing_account_id = seed_account_ledger()
        server, callback_url, received = _start_recorder()
        try:
            subscriptions = WebhookSubscriptionService(ledger, clock=lambda: ISSUED_AT)
            registered = subscriptions.register_subscription(
                TENANT_ONE, callback_url, (EVENT_TYPE_SPEND_BUDGET_PUBLISHED,)
            )
            self.assertEqual(
                registered.webhook_subscription_outcome_code,
                WebhookSubscriptionOutcomeCode.ACCEPTED,
            )
            self.assertEqual(registered.event_type_codes, (EVENT_TYPE_SPEND_BUDGET_PUBLISHED,))
            self.assertEqual(validate_webhook_subscription(registered.as_contract_dict()), ())
            first = SpendBudgetService(ledger, clock=lambda: ISSUED_AT).publish_spend_budget(
                TENANT_ONE, billing_account_id, "USD", BUDGET_AMOUNT, MORNING_WINDOW
            )
            second = SpendBudgetService(ledger, clock=lambda: ISSUED_AT).publish_spend_budget(
                TENANT_ONE, billing_account_id, "USD", BUDGET_AMOUNT, MORNING_WINDOW
            )
            published_events = [
                event
            ...
            self.assertEqual(first.spend_budget_outcome_code.value, "accepted")
            self.assertEqual(second.spend_budget_outcome_code.value, "duplicate_replay")
            self.assertEqual(len(published_events), 1)
            self.assertEqual(published_events[0].source_id, first.spend_budget_id)
            envelope = json.loads(published_events[0].payload_json)
            self.assertEqual(envelope["event_type_code"], EVENT_TYPE_SPEND_BUDGET_PUBLISHED)
            data = envelope["data"]
            self.assertEqual(data["spend_budget_id"], str(first.spend_budget_id))
            self.assertEqual(data["billing_account_id"], str(billing_account_id))
            self.assertEqual(data["source_payload_hash"], first.source_payload_hash)
            self.assertEqual(data["spend_budget_contract_version"], 1)
            self.assertEqual(data["currency_code"], "USD")
            self.assertEqual(data["budget_amount"], first.as_contract_dict()["budget_amount"])
            self.assertEqual(data["spend_budget_status"], "published")
            self.assertEqual(data["published_at"], first.as_contract_dict()["published_at"])
            self.assertNotIn("rated_amount", data)
            self.assertNotIn("remaining_amount", data)
            self.assertNotIn("over_amount", data)
            self.assertNotIn("utilization_status", data)
            self.assertNotIn("card_pan", json.dumps(envelope))
            self.assertNotIn("legal_invoice_number", json.dumps(envelope))
            self.assertNotIn("webhook_secret", json.dumps(envelope))
            delivered = WebhookDeliveryService(ledger).deliver_due_events(TENANT_ONE)
            ...
            headers, raw_body = received[0]
            posted = json.loads(raw_body.decode("utf-8"))
            self.assertEqual(posted["event_type_code"], EVENT_TYPE_SPEND_BUDGET_PUBLISHED)
            expected = sign_webhook_body(registered.webhook_secret or "", raw_body)
            signature = next(
                value
                for key, value in headers.items()
            ...
            self.assertEqual(signature, expected)
            orphan_id = published_events[0].outbox_event_id
            identity = next(
                key
                for key, stored_id in ledger.webhook_outbox_identity_index.items()
                if stored_id == orphan_id
            ...
            del ledger.webhook_outbox_events[orphan_id]
            del ledger.webhook_outbox_identity_index[identity]
            healed = SpendBudgetService(ledger, clock=lambda: ISSUED_AT).publish_spend_budget(
                TENANT_ONE, billing_account_id, "USD", BUDGET_AMOUNT, MORNING_WINDOW
            )
            healed_events = [
                event
            ...
            self.assertEqual(len(healed_events), 1)
            self.assertEqual(healed_events[0].source_id, first.spend_budget_id)
            rejected = SpendBudgetService(ledger).publish_spend_budget(
