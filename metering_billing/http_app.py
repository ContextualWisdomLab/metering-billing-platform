...
12. Let an operator drain AIS ``posting_receipt`` outbox events.  Empty
    ...
    compose a journal.
    ``POST /v1/spend-budgets/{spend_budget_id}/approaching-signal`` observes
    that published budget with the same remaining/over math and enqueues
    one existing ``spend_budget.approaching`` outbox event when
    utilization is ``at`` the documented ``budget_amount`` threshold.
    Same tenant is HTTP 200.  Unknown or cross-tenant is HTTP 404.
    Missing tenant pin is HTTP 422.  A budget whose billing account
    belongs to another commercial ``tenant_account`` is HTTP 403.  under
...
LATE_ADJUSTMENT_INVOICE_ADJUSTMENT_NESTED_PATH = re.compile(
    r"^/v1/late-adjustments/([0-9a-fA-F-]{36})/invoice-adjustments$"
)
...
def create_http_app(
    ...
    *ledger* accepts either the deterministic :class:`MemoryUsageLedger`
    ...
    late_adjustment_invoice_adjustments = LateAdjustmentInvoiceAdjustmentService(
        shared_ledger, clock=clock
    )
    ...
    def application(environ: WSGIEnvironment, start_response: StartResponse) -> Iterable[bytes]:
        ...
        if route_name == "late_adjustment_applications":
            try:
                payload = _read_json_object(environ)
                if FORBIDDEN_PAYMENT_INTENT_KEYS.intersection(payload):
                    raise HttpRequestError("request_invalid")
                tenant_reference = _authorized_tenant(environ, payload)
                result = late_adjustment_applications.apply_late_adjustment(
                    tenant_reference,
                    _parse_uuid(
                        path_values["late_adjustment_id"], "late_adjustment_id"
                    ),
        ...
        if route_name == "late_adjustment_ratings":
            try:
                ...
                return _send_json(start_response, 422, {"rejection_reason_code": "request_invalid"})
        if route_name == "late_adjustment_invoice_adjustments":
            try:
                payload = _read_json_object(environ)
                if FORBIDDEN_PAYMENT_INTENT_KEYS.intersection(payload):
                    raise HttpRequestError("request_invalid")
                tenant_reference = _authorized_tenant(environ, payload)
                result = late_adjustment_invoice_adjustments.record_invoice_adjustment(
                    tenant_reference,
                    _parse_uuid(
                        path_values["late_adjustment_id"], "late_adjustment_id"
                    ),
        ...
        if route_name == "spend_budget_over_signals":
            try:
                ...
                return _send_json(start_response, 422, {"rejection_reason_code": "request_invalid"})
        if route_name == "spend_budget_approaching_signals":
            try:
                payload = _read_json_object(environ)
                if FORBIDDEN_PAYMENT_INTENT_KEYS.intersection(payload):
                    raise HttpRequestError("request_invalid")
                tenant_reference = _authorized_tenant(environ, payload)
                result = spend_budget_approaching_signals.observe_spend_budget_approaching(
                    tenant_reference,
                    _parse_uuid(path_values["spend_budget_id"], "spend_budget_id"),
                )
                if result.spend_budget_approaching_signal_outcome_code.value == "rejected":
                    reason = result.rejection_reason_text()
                    if reason == "spend_budget_not_found":
                        status_code = 404
                    elif reason == "billing_account_not_found":
...
def _resolve_route(method: str, path: str) -> tuple[str | None, dict[str, str]]:
    ...
    if path == LATE_ADJUSTMENT_COLLECTION_PATH:
        ...
        return "method_not_allowed", {}
    late_adjustment_application_match = LATE_ADJUSTMENT_APPLICATION_NESTED_PATH.fullmatch(path)
    if late_adjustment_application_match is not None:
        if method == "POST":
    ...
    if late_adjustment_rating_match is not None:
        ...
        return "method_not_allowed", {}
    late_adjustment_invoice_adjustment_match = (
        LATE_ADJUSTMENT_INVOICE_ADJUSTMENT_NESTED_PATH.fullmatch(path)
    )
    if late_adjustment_invoice_adjustment_match is not None:
        if method == "POST":
            return "late_adjustment_invoice_adjustments", {
                "late_adjustment_id": late_adjustment_invoice_adjustment_match.group(1)
            }
        return "method_not_allowed", {}
