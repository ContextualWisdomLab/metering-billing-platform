...
def validate_provider_capability(
    ...
    errors = list(validate_schema_instance(schema, manifest))
    if not isinstance(manifest, Mapping) or manifest.get("effective_to") is None:
        return tuple(errors)
    try:
        effective_from = parse_iso8601_datetime(manifest.get("effective_from"))
        effective_to = parse_iso8601_datetime(manifest.get("effective_to"))
