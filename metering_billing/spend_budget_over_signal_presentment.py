"""Tenant-scoped over-signal presentment from live evaluation and stored outbox.
...
2. Project the live observation into the existing over-signal envelope.
3. Project zero or one stored ``spend_budget.over`` webhook-outbox
...
class SpendBudgetOverSignalPresentmentResult:
    """Buyer-facing live over-signal plus stored first-over outbox rows."""

    over_signal: SpendBudgetOverSignalResult
    ...
    def as_contract_dict(self) -> dict[str, object]:
        ...
        return {
            "spend_budget_over_signal_presentment_contract_version": (
                SPEND_BUDGET_OVER_SIGNAL_PRESENTMENT_CONTRACT_VERSION
            ),
            "over_signal": self.over_signal.as_contract_dict(),
...
class SpendBudgetOverSignalPresentmentService:
    """Read-only projector of one published budget's over-signal observation."""
    ...
    def present_spend_budget_over_signal(
        self, tenant_reference: str, spend_budget_id: UUID
    ) -> SpendBudgetOverSignalPresentmentResult:
        """Return one same-tenant live over-signal and stored outbox rows.
        ...
        try:
            ...
            raise SpendBudgetOverSignalPresentmentQueryError(
        ...
        if budget is None:
            raise ValueError("accepted over-signal presentment requires a stored budget")
        over_signal = _from_evaluation(
            evaluation,
        ...
        return SpendBudgetOverSignalPresentmentResult(over_signal, outbox)
