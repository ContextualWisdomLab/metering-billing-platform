...
- a durable PostgreSQL usage-to-issued-invoice, collection, payment, cash-journal, and webhook-delivery vertical: rate-card versions, rating runs, invoice drafts, issued-invoice lines, tenant-scoped tax rates and assessments, collection cases and dunning events, payment intents, applied payment receipts with row-locked settlement, balanced cash journal proposals, tenant-scoped webhook subscriptions, delivery attempts, delivered status, and atomic commercial outbox replay paths;
