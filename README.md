...
- a durable PostgreSQL usage-to-issued-invoice vertical: rate-card versions, rating runs, invoice drafts, issued-invoice lines, tenant-scoped tax rates and assessments, and an atomic `invoice.issued` webhook outbox replay path;
...
controls are complete. The durable vertical ends at the issued-invoice
snapshot and its atomic `invoice.issued` outbox event; issue #84 remains open.
