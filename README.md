...
dimensions, verify Lemon Squeezy webhook raw-body signatures before
normalizing a PII-free event reference, and keep provider object mappings
sticky unless an explicit replacement is recorded. The included Lemon Squeezy
and manual enterprise manifests are declarations only. Provider command ports,
KMS-backed secrets, raw-artifact retention, manual maker-checker settlement,
and live provider sandbox evidence remain open under issue #86.
