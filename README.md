...
- an importable `metering_billing` package that ingests immutable usage, deduplicates by source-event key plus source-payload hash and contract version, and rates tenant-scoped half-open windows into exact invoice-intent totals;
...
```bash
python3 -c "from metering_billing import UsageRatingService, TimeWindow"
```

After usage is stored, call `UsageRatingService.rate_usage_window` with a tenant URN, a half-open ISO 8601 window, and a rate-card version. Only `billable` quality enters the invoice-intent total. A replay of the same tenant, window, rate-card version, and usage snapshot returns the same `rating_run_id` and exact totals. Rating does not draft an invoice or post a journal.

## Next action

After rating merges, implement invoice draft from persisted rating runs before adding a payment-provider adapter. Do not post accounting journals from billing.
