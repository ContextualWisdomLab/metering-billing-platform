...
For a worker, construct `AisOutboxScheduler(drain_service, tenant_references, interval_seconds=60, stop_event=stop_event, on_cycle=record_results)`. The tenant source is called at the start of every cycle; `run_once()` returns each tenant's drain result, while `run_forever()` forwards the same tuple to the optional `on_cycle` observer and exits promptly when the supplied stop event is set. The scheduler adds no delivery state or monetary side effect.
