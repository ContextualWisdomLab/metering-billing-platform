...
- an importable `metering_billing` package that ingests immutable usage, publishes versioned rate cards, rates tenant-scoped half-open windows against a persisted version, drafts invoice intent, issues commercial invoices, issues commercial credit notes, voids unused issued credit notes, publishes tax rates, assesses tax on a draft, emits proposal-only journals, opens commercial collection cases, presents collection aging, projects provider-neutral payment intents, applies commercial payment receipts, records commercial credits, pulls AIS posting receipts as observations, drains AIS posting-receipt outbox events, issues tenant API credentials, registers webhook callbacks for accepted commercial facts, and accepts those writes over a stdlib HTTP adapter;
