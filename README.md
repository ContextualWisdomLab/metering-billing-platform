...
- a durable PostgreSQL usage-to-issued-invoice and webhook-delivery vertical: rate-card versions, rating runs, invoice drafts, issued-invoice lines, tenant-scoped tax rates and assessments, tenant-scoped webhook subscriptions, delivery attempts, delivered status, and an atomic `invoice.issued` outbox replay path;
